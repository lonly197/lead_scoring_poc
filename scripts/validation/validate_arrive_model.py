...
TASK_NAME = "validate_arrive_model"
...
def infer_model_type(model_path: Path) -> str | None:
    ...
    if str(metadata.get("label")) == "到店标签_14天":
        return "arrive"
    return None


def validate_arrive_model_artifacts(metadata: dict, model_path: Path) -> None:
    pipeline_metadata = metadata.get("pipeline_metadata", {})
    if pipeline_metadata.get("pipeline_mode") == "two_stage":
        raise RuntimeError(f"检测到 OHAB 两阶段产物，当前路径不是到店模型: {model_path}")
...
def find_available_model(default_path: Path) -> Path:
    ...
    if models_dir.exists():
        for name in ["arrive_model", "arrive_oot"]:
            candidate = models_dir / name
            if candidate.exists() and (candidate / "predictor.pkl").exists():
                return candidate
        for model_dir in models_dir.iterdir():
            if model_dir.is_dir() and infer_model_type(model_dir) == "arrive":
                return model_dir
    return default_path
...
def parse_args():
    parser = argparse.ArgumentParser(description="验证到店预测模型")
    parser.add_argument("--log-file", type=str, default=None, help="日志文件路径")
    parser.add_argument("--model-path", type=str, default="outputs/models/arrive_model", help="模型路径")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/validation/arrive_validation", help="输出目录")
...
def main():
    ...
    try:
        ...
        if not model_path.exists():
            raise RuntimeError(f"未找到到店模型目录: {args.model_path}")

        metadata = load_feature_metadata(model_path)
        validate_arrive_model_artifacts(metadata, model_path)
        ...
        if split_info:
            ...
            if mode in {"oot", "oot_manual"} and "线索创建时间" in df.columns:
                df["线索创建时间"] = pd.to_datetime(df["线索创建时间"], errors="coerce")
                valid_end = pd.Timestamp(split_info.get("valid_end"))
                df = df[df["线索创建时间"] >= valid_end].copy()
            elif mode == "random":
                if "test_ids" in split_info and "id_column" in split_info:
                    df = df[df[split_info["id_column"]].isin(set(split_info["test_ids"]))].copy()
                elif "test_indices" in split_info:
                    df = df.iloc[split_info["test_indices"]].copy()
