...
def load_arrive_validate_script(monkeypatch):
...
def test_arrive_artifacts_accept_minimal_feature_metadata(monkeypatch):
    validate_script = load_arrive_validate_script(monkeypatch)

    validate_script.validate_arrive_model_artifacts(
        {
            "split_info": {"mode": "random"},
        },
        Path("outputs/models/arrive_model"),
    )


def test_arrive_artifacts_reject_hab_pipeline_metadata(monkeypatch):
    validate_script = load_arrive_validate_script(monkeypatch)
...
def test_infer_arrive_model_type_from_path(monkeypatch):
    validate_script = load_arrive_validate_script(monkeypatch)
