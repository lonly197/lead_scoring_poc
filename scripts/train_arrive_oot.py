...
使用新数据格式，预测 7 天内到店，采用三层 OOT 时间切分：
- 训练集: 3月1日-3月11日
- 验证集: 3月11日-3月16日
- 测试集: 3月16日-3月23日（有完整 7 天观察期）
...
TASK_NAME = "train_arrive_oot"


def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="到店预测训练（OOT验证）")
...
def main():
    ...
    output_dir = Path(args.output_dir or "./outputs/models/arrive_oot_7d")
    ...
    try:
        ...
        # 下一阶段数仓导出标准格式数据后，可改为 auto_adapt=False
        ...
        logger.info(f"处理后: {len(df_processed):,} 行, {len(df_processed.columns)} 列")
        ...
        # 验证集评估
        logger.info("【验证集评估】")
        valid_proba = predictor.get_positive_proba(valid_df)
        valid_metrics = compute_topk_metrics(
            y_true=valid_df[target_label].values,
            y_proba=valid_proba,
            k_values=[100, 500, 1000],
        )
        logger.info(f"验证集 Top-K: {valid_metrics}")

        # 测试集评估
        logger.info("【测试集评估】")
        test_proba = predictor.get_positive_proba(test_df)
        test_metrics = compute_topk_metrics(
            y_true=test_df[target_label].values,
            y_proba=test_proba,
            k_values=[100, 500, 1000, 5000],
        )
        logger.info(f"测试集 Top-K: {test_metrics}")

        # 计算基线和 Lift
        baseline = test_df[target_label].mean()
        logger.info(f"基线转化率: {baseline*100:.2f}%")

        for k, metrics in test_metrics.items():
            lift = metrics["lift"]
            logger.info(f"{k}: 命中率 {metrics['hit_rate']*100:.2f}%, Lift = {lift:.2f}x")
        ...
        report_path = output_dir / "reports" / "oot_evaluation_report.json"
        ...
        print("训练完成")
        print("=" * 60)
        print(f"测试集 AUC: {predictor.evaluate(test_df):.4f}")
        print(f"Top-100 命中率: {test_metrics['top_100']['hit_rate']*100:.1f}%")
        print(f"Top-100 Lift: {test_metrics['top_100']['lift']:.2f}x")
        print(f"\n日志文件: {log_file}")
