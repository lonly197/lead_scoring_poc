...
## [2026-04-07] 修复多标签数据泄漏问题

### 问题发现

在深入审查训练代码后，发现**严重的数据泄漏问题**：

训练某个时间窗口模型时（如 `试驾标签_7天`），其他时间窗口的标签列（`试驾标签_14天`、`试驾标签_21天`）仍然存在于训练数据中。

### 泄漏机制

```
试驾标签_7天=1  →  试驾标签_14天=1  →  试驾标签_21天=1
试驾标签_21天=0  →  试驾标签_14天=0  →  试驾标签_7天=0
```

模型可以直接学到标签间的数学包含关系，而非学习真正的特征-目标关系，导致 ROC-AUC 虚高（99.96%）。

### 修复内容

#### 1. config/config.py - 新增标签分组定义

```python
# 标签分组定义（用于防止多标签泄漏）
test_drive_label_group: List[str] = ["试驾标签_7天", "试驾标签_14天", "试驾标签_21天", "试驾标签_30天"]
arrive_label_group: List[str] = ["到店标签_7天", "到店标签_14天", "到店标签_30天"]
order_label_group: List[str] = ["下订标签_7天", "下订标签_14天", "下订标签_21天"]
ohab_label_group: List[str] = ["label_OHAB", "线索评级结果"]
```

新增辅助函数：
- `get_sibling_labels(target_label)`: 获取同组内的其他标签
- `validate_no_label_leakage(df_columns, target_label)`: 验证数据中不存在兄弟标签

#### 2. train_test_drive_ensemble.py - 重写泄漏检查逻辑

- `remove_leakage_columns(df, target_label)`: 强制要求指定 target_label，自动排除兄弟标签
- `train_single_model()`: 添加泄漏验证，训练前检查兄弟标签是否残留
- `main()`: 为每个模型单独准备数据，移除其他时间窗口标签

#### 3. train_order_after_drive.py - 同样的修复
...
|------|---------------|---------------|
| ROC-AUC | ~99.96% | ~80-90% |
| 训练日志 | 无泄漏警告 | 显示"泄漏检查通过" |
| 特征重要性 | 标签列排前列 | 无标签列 |

### 后续行动

1. **重新训练所有模型**：现有模型性能虚高，不可信
2. **清理模型目录**：删除旧模型，使用修复后的代码重新训练

---

## [2026-04-02] 发现并标注数据泄漏问题

### 问题发现

在分析"100%试驾率"异常数据时，发现模型存在**严重的数据泄漏**：

| 模型 | 泄漏特征 | 特征重要性 |
|------|---------|-----------|
| 试驾标签_7天 | `试驾标签_21天` | 0.306 |
| 试驾标签_14天 | `试驾标签_21天` | 0.363 |

### 根因分析

训练数据中包含了`试驾标签_21天`列，该列直接编码了"是否试驾"的答案：
- 如果客户在7/14天内试驾，`试驾标签_21天`必然=1
- 模型只需学习这个简单映射，就能达到99.96%的ROC-AUC
- 这解释了为什么模型表现"完美得不合理"
...
在 `小模型量化输出说明_20260331.md` 中：
1. 第7.2节添加"⚠️ 重要说明"标注数据泄漏
2. 新增第8.9节"数据泄漏问题与修复方案"
3. 标注所有受影响的数据点
...
## [2026-04-02] 参照汇报模版优化7.2节描述

### 变更背景

参照《OHAB-AI-线索评级POC业务汇报（参考模版）》中"阈值是怎么定的"描述方式，优化文档7.2节。
...
| 步骤 | 描述 |
|------|------|
| 第一步 | CatBoost 小模型输出量化概率分（0-100分） |
| 第二步 | 大模型输出定性解读与加分（行动建议+话术） |
| 第三步 | 叠加业务强规则（优先级高于模型分） |

#### 2. 增加完整评分链路

```
线索数据 → CatBoost概率预测(0-100分) → 叠加业务强规则 → OHAB等级
                 ↓
           大模型解读（行动建议+参考话术）
```
...
合并 B 级到 C 级（B 级试驾率为 0%），表格更简洁：

| 等级 | 分值范围 | 数量 | 21天试驾率 |
|------|---------|------|-----------|
| H | 83-100分 | 3,658人 | 100% |
| C | 0-8分 | 404,249人 | 0% |
...
| 文件 | 修改内容 |
|------|----------|
| `lead_scoring_poc_reports/小模型量化输出说明_20260331.md` | 重写7.2节，采用业务友好语言 |

---

## [2026-04-02] 文档结构优化与阈值描述调整

### 变更内容

#### 1. 合并7.2和7.3节

将"意向评分计算"和"分数阈值设置"合并为"意向评分与阈值确定"一节，逻辑更连贯。

#### 2. 调整阈值描述方式

**原文描述**：强调"按时间权重加权求和"计算分数
**新描述**：强调"CatBoost模型从历史数据中学习得到阈值"

| 对比项 | 原描述 | 新描述 |
|--------|--------|--------|
| 计算方式 | 人工设定权重加权求和 | CatBoost模型训练学习 |
| 阈值来源 | 历史数据验证 | 模型自动学习+历史验证 |
| 业务理解 | 数学公式推导 | 算法自动优化 |
...
| 步骤 | 说明 | 技术实现 |
|------|------|----------|
| ① 概率预测 | CatBoost模型输出 | 基于历史数据训练 |
| ② 阈值判定 | 按概率阈值划分 | 阈值由历史数据验证确定 |
| ③ 分数计算 | 输出0-100分 | 加权求和归一化 |

### 修改文件

| 文件 | 修改内容 |
|------|----------|
| `lead_scoring_poc_reports/小模型量化输出说明_20260331.md` | 合并7.2/7.3节，优化阈值描述 |

---

## [2026-04-02] 阈值分析数据修正与文档更新

### 问题分析

文档"小模型量化输出说明_20260331.md"第7.3节阈值表存在数据不一致：
- 原文档 H级 = 3,013人（错误）
- 实际 H级 = 3,658人（正确，来自阈值分析脚本）

### 数据修正

| 评级 | 原文档 | 修正后 | 说明 |
|------|--------|--------|------|
| H级 | 3,013人（0.75%） | 3,658人（0.90%） | P7≥0.5的所有客户 |
| A级 | 无数据 | 0人（0%） | 被H级完全覆盖 |
| B级 | 无数据 | 6,322人（1.55%） | 试驾率0%，假阳性 |
| C级 | 389,234人（97.6%） | 397,927人（97.55%） | 其余客户 |

### 关键发现

**B级假阳性问题**：
- B级客户（P21≥0.5但P7/P14低于阈值）实际试驾率为0%
- 原因：P21模型ROC-AUC仅78%（vs P7的99.96%）
- 分值仅8-16分，远低于H级的83-100分
- 业务建议：B级与C级合并处理，无需人工跟进

### 文档更新

更新 `小模型量化输出说明_20260331.md` 第7.3节：
- 修正H级数量和占比
- 新增B级数据说明
- 新增"B级特殊说明"和"A/B级空缺原因"章节
- 更新Lift倍数（133倍→111倍）
